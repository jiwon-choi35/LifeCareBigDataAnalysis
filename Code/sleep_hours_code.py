...
#  1. 실제 다운로드 받은 2020년~2024년 SAS 파일의 경로 리스트입니다. (사용자 지정 경로)
# 이 경로에 파일이 존재해야 분석이 실행됩니다.
file_paths = [
...
VAR_SLEEP = 'mtc_10z1'           # 독립 변수: 실제로 잠잔 시간(시)
...
# 수면 시간 유효값(77 미만) 및 모든 통제 변수의 유효값(1 또는 2) 확인
...
# 3-2.  통제 변수 리코딩 
df_analysis['male'] = df_analysis[VAR_SEX].replace({2: 0, 1: 1}).astype(int) # 여성(2)을 기준(0)으로
...
# ---------------------------------------------
# --- 4. 로지스틱 회귀 분석 실행 및 결과 출력 (주석 해제됨) ---
# ---------------------------------------------

#  모델 공식: 수면 시간 + 통제 변수 3가지 모두 포함 
formula = (
...
print("\n--------------------------------------------------")
print("3. 📉 다중 로지스틱 회귀 분석 최종 결과 (수면 시간 + 통제 요인)")
print("--------------------------------------------------")
